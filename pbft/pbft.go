...
func NewPBFT(nodeID, addr string) *pbft {
	...
	p.node.addr = addr
	p.node.rsaPrivKey = getPivKey(nodeID) //从生成的私钥文件处读取
	p.node.rsaPubKey = getPubKey(nodeID)  //从生成的私钥文件处读取
	p.sequenceID = 0
...
func (p *pbft) handleClientRequest(content []byte) {
	...
	//主节点对消息摘要进行签名
	sig := Sign(digest, p.node.nodeID)
	//拼接成PrePrepare，准备发往follower节点
	pp := PrePrepare{*r, digest, p.sequenceID, sig}
...
func (p *pbft) handlePrePrepare(content []byte) {
	...
	if p.sequenceID+1 != pp.SequenceID {
		fmt.Println("消息序号对不上，拒绝进行prepare广播")
		return
	}

	//获取主节点的公钥，用于数字签名验证
	if err := Verify(pp.Digest, pp.Sign, "N0"); err != nil {
	...
	//节点使用私钥对其签名
	sign := Sign(pp.Digest, p.node.nodeID)
	//拼接成Prepare
...
func (p *pbft) handlePrepare(content []byte) {
	...
	if p.sequenceID != pre.SequenceID {
		fmt.Println("消息序号对不上，拒绝执行commit广播")
		return
	}
	if err := Verify(pre.Digest, pre.Sign, pre.NodeID); err != nil {
	...
	if count >= specifiedCount && !p.isCommitBordcast[pre.Digest] {
		...
		//节点使用私钥对其签名
		sign := Sign(pre.Digest, p.node.nodeID)
...
func (p *pbft) handleCommit(content []byte) {
	...
	if err := Verify(c.Digest, c.Sign, c.NodeID); err != nil {
